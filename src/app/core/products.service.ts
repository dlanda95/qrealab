...
export class ProductsService {
  ...
  getCategories(): Observable<ProductCategory[]> {
    return this.http
      .get<any>(`${this.base}/api/product-categories?sort=_order&limit=100`)
      .pipe(map(res => res.docs.map((d: any) => ({ id: d.id, name: d.name, slug: d.slug }))));
  }

  getProducts(): Observable<Product[]> {
    return this.http
      .get<any>(`${this.base}/api/products?depth=1&where[status][equals]=active&sort=_order&limit=200`)
      .pipe(map(res => res.docs.map((d: any) => this.mapProduct(d))));
  }

  getProductBySlug(slug: string): Observable<Product | null> {
    return this.http
      .get<any>(`${this.base}/api/products?depth=1&where[slug][equals]=${slug}&limit=1`)
